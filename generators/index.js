...
module.exports = Generator.extend({
    ...
    writing: function () {
        ...
        for (var definition in generator.api.definitions) {
            log('Creating ' + generator.api.definitions[definition]['modelName'] + '...');
        }
        ...
        for (var component in generator.api.components) {
            ...
            log('Creating ' + componentName + '...');
        }

        // Creating actions
        for(var tag in generator._tags) {
            log('Creating actions and effects for:' + chalk.red(generator._tags[tag]) + '');
            for (var path in generator.api.paths) {
                for (var method in generator.api.paths[path]) {
                    if (generator._tags[tag] == generator.api.paths[path][method].tags[0]) {
                        ...
                        log('\tFor operation:' + chalk.red(basename) + '');
                        ...
                        log('\t\t' + chalk.red(actionname) + '');
                        log('\t\t' + chalk.red(effectname) + '');
        ...
        log(chalk.blue('[es6 module]') + chalk.green('[es6 module]') + 'Creating modules');
        for(var tag in generator._tags) {
            ...
            log('\tCreating module: ' + chalk.yellow(componentname) + '');
            ...
            for (var path in generator.api.paths) {
                for (var method in generator.api.paths[path]) {
                    if (generator._tags[tag] == generator.api.paths[path][method].tags[0]) {
                        ...
                        log('\t\tCreating Command Class ' + chalk.yellow(commandname) + '');
                    }
                }
            }
        }
